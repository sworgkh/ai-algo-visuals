...
- **Topic 10 — Language Models.** Tested bigram engine `src/lib/langmodel.ts` (+ `langmodel.test.ts`, 11 tests): `buildBigram` (with <s>/</s> padding), `mle`, add-α `laplace`, `sentenceSteps`/`sentenceProb` (chain rule under the Markov assumption), `perplexity` (∞ on any zero step), and `nextWordDist`. Asserts counts, that P(·|ctx) sums to 1, that an unseen bigram is 0 under MLE but positive under Laplace, and that perplexity is ∞ under MLE yet finite once smoothed. Three tabs (`?tab=`): **N-gram model** (context chips → next-word bar distribution with counts, α slider, chain-rule KaTeX), **Score a sentence** (per-step P(wᵢ|wᵢ₋₁) chain, total probability + perplexity, unseen bigrams flagged red, smoothing toggle), **Perplexity & smoothing** (perplexity-vs-α curve — ∞ at MLE, finite once smoothed — with honest in-corpus-vs-held-out framing, plus Laplace/backoff/interpolation cards). All use VizGuide. **Portal complete: all 10 topics built.**
